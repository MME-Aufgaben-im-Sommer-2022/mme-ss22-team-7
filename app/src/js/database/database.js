...
let api = {
  ...
  createUserDocument: (userId, data, read, write) => {
    return database.createDocument(Server.userCol, userId, data, read, write);
  },

  myDocument2: () => {
    return database.listDocuments(Server.databaseID, Server.userCol, [
  ...
  updateChallenges: (active, challengesTimes, completed) => {
    return database.updateDocument(Server.userCol, userID, {
      CompletedChallenges: completed,
      ActiveChallengesTime: challengesTimes,
